  ...
  */ 
  ...
  public:
    ...
    */
    ...
    {
      assert(pool);
      work_mem_ptr = malloc(WORKSPACE_MEM);
    }

      
     ~worker_thread()
     {
        if(NULL!=work_mem_ptr)
             free((void*)work_mem_ptr);
     }
	  void run()
	  { 
		  scope_guard notify_exception(bind(&worker_thread::died_unexpectedly, this));

		  while(m_pool->execute_task((void*)work_mem_ptr)) {}
